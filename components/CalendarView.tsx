  ...
  return (
    <div className="pb-4 animate-in fade-in duration-500">
      <div className="bg-dark-800 rounded-3xl overflow-hidden border border-dark-700 shadow-xl shadow-black/20">
        ...
        <div className="grid grid-cols-7 bg-dark-800">
          {days.map((day, i) => {
            ...
            else if (hasMatch) bgClass = "bg-brand-teal/5 shadow-inner shadow-brand-teal/5 cursor-pointer hover:bg-brand-teal/10";
            else bgClass = "cursor-pointer hover:bg-dark-700/50";
      ...
      <MatchDetailsModal
        ...
        date={selectedDate}
        matches={selectedDate ? matches.filter(m => m.date === format(selectedDate, 'yyyy-MM-dd')) : []}
      />
    </div>
  );
};